...
Every command below was executed as written against small synthetic stages laid out exactly like the data
directory of section 2; the only thing you change is having the real files there.
...
| score locally on a pseudo split | 7 | additionally the organisers' scorer **veckit** and its dependencies (numpy, scipy, anndata, scikit-learn). Obtain it from the organisers: `pip install git+https://github.com/aristoteleo/veckit.git`, or `git clone https://github.com/aristoteleo/veckit` anywhere and point `VECKIT_PATH` at the clone (`set VECKIT_PATH=C:\path\to\veckit` on Windows, `export VECKIT_PATH=/path/to/veckit` elsewhere). Check: `python -c "from vec_local_score import veckit_available, veckit_info; print(veckit_available(), veckit_info())"`. The kit was tested against veckit 0.1.1. |
...
Rules of a valid file (`vec_submit_check` tests these locally; the portal's own validator has the final say):
...
4. `.X` finite, castable to float32, **non-negative on every board**, log-normalised like the released data.
...
The released stages hold more cells than four of the five boards allow, so `all` would stop with an error that
states the bound; pass a number. The bound is written next to each command (from `data/panels/index.json`).
...
* The CLI prints the board-contract verdict for the written file and exits 0 only when it passes, e.g.
...
Exit code 0 = passes, 1 = fails, 2 = usage error (unknown board or missing file). This is a set of local
pre-upload checks against the published contracts, not a reproduction of every portal rule: the portal's validator
has the final say, but a file that fails here would have failed there, so checking first reduces avoidable
upload-and-debug round trips. The most common failures: gene order differs (`reorder with the panel file`),
...
ones, and run the organisers' scorer on that. `vec_local_score` wraps veckit for this and follows the veckit
...
`vec_agent_evidence` makes all of this mechanical. The current implementation targets the Claude Code CLI in
...
- [ ] Cell count inside the board bounds (you passed it explicitly); no duplicated cells; no labels needed.
